...
public class Club {
	...
	public boolean addJugador(Jugador jugador) {
		...
		if(posicionUltimoJugador()>=JUGADORES) {
			fichado=false;
		}else {
			if(jugador instanceof Portero) {
				if(contadorP>=PORTEROS) {
					fichado=false;
				}else {
					fichado=true;
					contadorP++;
	...
	private void actualizarEstadisticasJugadores(Partido partido) {
        for (Jugador jugador : jugadores) {
            if (jugador != null) {
                if (partido.getVisitante().equals(nombre)) {
                ...
                } else if (partido.getLocal().equals(nombre)) {
                	 if (jugador instanceof Delantero) {
                     	Delantero delantero=(Delantero) jugador;
