...
while opcion!="g":
    ...
    if opcion=="A":
        nclientes+=1
        tipocuentaP=bool(input("Quiere una cuenta premium(True or false)."))
...
while opcion!="g":
    ...
    if opcion=="A":
        ...
        sumacliente=udidadesProducto*precioproductoUnitario
        if sumacliente>sumacliente:
            sumamejorcliente=sumacliente
            correoMejorCliente=correoelec
        if tipocuentaP==True:            
            nclientesP+=1
            if precioproductoUnitarioMasCaroP<precioproductoUnitario:
                precioproductoUnitarioMasCaroP=precioproductoUnitario
                codigoProductoMasCaroP=codigoProducto
                correoMejorClienteP=correoelec
        else:
            nclientesN+=1
            if precioproductoUnitarioMasCaroN<precioproductoUnitario:
                precioproductoUnitarioMasCaroN=precioproductoUnitario
                codigoProductoMasCaroN=codigoProducto
                correoMejorClienteN=correoelec
        if precioproductoUnitarioMasCaroN<precioproductoUnitarioMasCaroP:
            precioproductoUnitarioMasCaro=precioproductoUnitarioMasCaroP
            correoMejorCliente=correoMejorClienteP
        elif precioproductoUnitarioMasCaroN>precioproductoUnitarioMasCaroP:
            precioproductoUnitarioMasCaro=precioproductoUnitarioMasCaroN
            correoMejorCliente=correoMejorClienteN
    elif opcion=="B":
        print(f"El {(nclientesP/nclientes)*100}% de los clientes son premium")
        print(f"El {(nclientesN/nclientes)*100}% de los clientes son normales")
    elif opcion=="C":
        ...
        print(f"El mejor cliente es {correoMejorCliente} y el importe gastado es de {sumamejorcliente}€")
    else:
        print("Opcion incorecta")
